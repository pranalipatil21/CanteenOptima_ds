 ...
 * @returns {Array} - Array of state objects for visualization
 */
export function generateDijkstraStates(nodes, edges, startNodeId) {
  ...
  // Build adjacency list for undirected graph (or directed if needed, assume undirected for now)
  const adjList = {};
  nodes.forEach(n => adjList[n.id] = []);
  edges.forEach(e => {
    adjList[e.from].push({ to: e.to, weight: e.weight });
    adjList[e.to].push({ to: e.from, weight: e.weight }); // Undirected
  });
  ...
  // Optional: We can add states to highlight the path to a specific target if requested, 
  // but for the base algorithm we just show the distances.
